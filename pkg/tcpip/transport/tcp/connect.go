...
// handleSegments processes all inbound segments in two modes. In fastPath
// mode it will drain it's inbound queue completely as the processor is
// responsible for handling in batches as needed. If fastPath is false
// then we only process in batches of maxSegmentsPerWake to ensure the
// endpoint is able to handle other events as well.
func (e *endpoint) handleSegments(fastPath bool) (err *tcpip.Error) {
	checkRequeue := true
	for i := 0; ; i++ {
		...
		s.decRef()
		if !fastPath && i > maxSegmentsPerWake {
			break
		}
	}
	...
	funcs := []struct {
		...
		},
		{
			...
			f: func() *tcpip.Error {
				...
				if n&notifyDrain != 0 {
					// On undrain/err restore the old Limit.
