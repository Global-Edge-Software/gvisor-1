...
// segmentEndpointEntry represents a segment and the endpoint to which
// the segment should be delivered.
...
func (p *processor) handleSegments() {
	...
	for {
		...
		for id, ep := range epMap {
			delete(epMap, id)
			ep.mu.RLock()
			epState := ep.EndpointState()
			workerRunning := ep.workerRunning
			ep.mu.RUnlock()

			// For sockets not in established state and sockets for
			// which the worker has not yet been started do not
			// bypass normal segment handling.
			...
			ep.handleSegments(true /* fastPath */)
			ep.workMu.Unlock()
		}
